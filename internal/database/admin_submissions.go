...
// projectNameSQL derives the project name from a users.repository column the
// way User.GetProjectName does, for joins with pipelines.
func projectNameSQL(repository string) string {
	return "regexp_replace(rtrim(" + repository + ", '/'), '^.*/', '')"
}
