...
func (s *server) handleAdminBanSubmission(c *gin.Context) {
	...
	admin := *s.getUser(c).GitlabLogin
	if err := s.db.BanSubmission(pipelineID, reason, admin); err != nil {
		c.String(http.StatusInternalServerError, "failed to ban submission")
		return
	}
	s.cache.Clear()
	s.logger.Info("Submission banned", zap.Int("pipeline_id", pipelineID), zap.String("admin", admin), zap.String("reason", reason))
...
func (s *server) handleAdminUnbanSubmission(c *gin.Context) {
	...
	found, err := s.db.UnbanSubmission(pipelineID)
	if err != nil {
		c.String(http.StatusInternalServerError, "failed to unban submission")
		return
	}
	if !found {
		c.String(http.StatusNotFound, "pipeline is not banned")
		return
	}
	s.cache.Clear()
